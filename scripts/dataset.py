...
try:
    ...
    from scripts.augmentation import SpatialAugmentation
except ImportError:
...
class ASLParquetDataset(Dataset):
    ...
    def __init__(self, csv_path='data/train.csv', data_root='data/', sign2ord_map=None, augment=False):
        ...
        if self.sign2ord_map is not None:
            ...
            if self.df['sign_ord'].isnull().any():
                missing_signs = self.df.loc[self.df['sign_ord'].isnull(), 'sign'].unique()
                raise ValueError(f"Missing sign mapping for: {missing_signs}")
            self.df['sign_ord'] = self.df['sign_ord'].astype(int)
        elif 'sign_ord' not in self.df.columns:
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            raw_data_torch = self.spatial_aug(raw_data_torch)
